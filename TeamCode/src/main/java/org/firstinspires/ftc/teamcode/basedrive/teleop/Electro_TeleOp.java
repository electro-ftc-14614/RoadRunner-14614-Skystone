...
public class Electro_TeleOp extends LinearOpMode {
    ...
    public void runOpMode() {
        ...
        double right2y;
        /* Automated Lift motor Section*/

        int topLevel = 0;
        int level = 0;
        boolean dPadUp2Lock = false;
        boolean dPadDown2Lock = false;
        ElapsedTime dPadUpTimer = new ElapsedTime(ElapsedTime.Resolution.MILLISECONDS);
        ElapsedTime dPadDownTimer = new ElapsedTime(ElapsedTime.Resolution.MILLISECONDS);
        ...
        while (opModeIsActive() && !isStopRequested()) {
            ...
            /**         LIFT MOTOR SECTION  **/
            if (gamepad2.dpad_up && level<=topLevel && !robot.isLiftMoving()) {
                if (dPadUpTimer.time(TimeUnit.MILLISECONDS) > 200) {
                   robot.resetFlipServoPosition();
                    ...
                    robot.moveLift(level*5, .8);//Moving Lift
//                    if (!robot.isLiftMoving()){
//                        robot.setTMMPower(-.5);
//                        this.sleep(200);
//                        robot.resetGripperServoPosition();
//                        robot.setTMMPower(0.5);
//                        this.sleep(200);
//                        robot.setTMMPower(0);
//                    }
                }
            ...
            else if (gamepad2.dpad_down && !robot.isLiftMoving()) {
                if (dPadDownTimer.time(TimeUnit.MILLISECONDS) > 200) {
                    dPadDownTimer.reset();
                    /////do stuff
                    level = 0;
                    robot.moveLift(level*5, .8);//Moving Lift
